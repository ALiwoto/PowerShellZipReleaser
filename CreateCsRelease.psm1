...
class CsProjectContainer {
    ...
    [void]ModifyProjectVersion([string]$TargetCsVersion) {
        ...
        if ($tmpRawContent.Contains("<version>") -and $tmpRawContent.Contains("</version>")) {
            $allStrs = $this.RawContent.Split(@("<Version>", "<version>", "</Version>", "</Version>"),`
                3, "RemoveEmptyEntries")
            $allStrs[1] = "<Version>$TargetCsVersion</Version>"
        ...
        $commentAttr = "<!--Version tag generated by ZipReleaser-->"
        $allStrs[0] += "<PropertyGroup>`n      $commentAttr`n      <Version>$TargetCsVersion</Version>`n"
        $this.RawContent = [string]::Join("", $allStrs)
        $this.RawContent | Out-File -FilePath $this.CsProjectFilePath
    }
}
